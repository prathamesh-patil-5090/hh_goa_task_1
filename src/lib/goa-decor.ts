...
/** Clean vector barcode for Team Code / Builder ID. */
export function drawBarcode(
  ...
  fontMono: string = '"Victor Mono", ui-monospace, monospace',
) {
  ctx.save();
  ctx.fillStyle = color;
  ctx.strokeStyle = color;
  ...
  const startX = cx - w / 2;
  const numBars = 36;
  const unitW = w / numBars;

  let hash = 0;
  for (let i = 0; i < codeText.length; i++) {
    hash = (hash * 31 + codeText.charCodeAt(i)) & 0xffffffff;
  }

  for (let i = 0; i < numBars; i++) {
    const bit = Math.abs((hash ^ (i * 2654435761)) >> (i % 24)) & 1;
    const isWide = ((hash >> (i % 16)) & 3) === 0;
    const barWidth = bit ? (isWide ? unitW * 1.6 : unitW * 0.9) : unitW * 0.35;
    const bx = startX + i * unitW + (unitW - barWidth) / 2;

    if (i % 3 !== 0) {
      ctx.fillRect(bx, cy, barWidth, barH);
    }
  }

  ctx.font = `800 16px ${fontMono}`;
  ctx.textAlign = "center";
  ctx.fillText(codeText.toUpperCase(), cx, cy + h - 2);
  ctx.restore();
}
